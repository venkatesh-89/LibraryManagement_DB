...
<form method="post" action="<%=context %>/ctr" onsubmit="return validateInput();">
	<table class="inputTable" border=0>
		...
		<tr>
			...
			<td><input type="text" name="searchBorrowerName" id="searchBorrowerName" size="30" /></td>
		</tr>
		<tr>
...
<%if(session.getAttribute("LoanSearchList")!=null)
	{
		...
		{
%>
		<font style="margin-left: 20px;font-size: 16px;color: blue;">Search results for <b><i><%=(String)session.getAttribute("searchLoan") %></i></b> :</font>
<%		} 
%>
...
%>
				<tr>
					...
					onclick="window.location.href='<%=context%>/ctr?action=Check_in&control=View&LoanId=<%=loanB.getLoanId()%>'"/></td>
				</tr>
<%			} 
%>
